...
- `ffai/src/ffai/data/raw/` — files downloaded directly from ESPN. Never modify these; only append new years.
- `ffai/src/ffai/data/processed/` — files derived from raw data (joins, aggregations, feature engineering). These are generated by scripts and should be reproducible from raw.

Both directories are gitignored (local only). When adding a processing step, write a script that reads from `raw/` and writes to `processed/` so the transformation is auditable.
