...
- `ffai/src/ffai/data/processed/` — files derived from raw data (joins, aggregations, feature engineering). Generated by `scripts/build_features.py`; reproducible from raw.

All three directories are gitignored (local only). When adding a processing step, write a script that reads from `favrefignewton/` or `nflverse/` and writes to `processed/` so the transformation is auditable.
